...
const services = [
  { ...tts('azure-speech', 'Azure AI Speech', 'https://{region}.tts.speech.microsoft.com/cognitiveservices/v1', 'tts', 'azureSpeechTts'), auth: 'ocp-key', fields: [{ name: 'region', label: 'Azure region', type: 'text', value: 'eastus' }, { name: 'voice', label: 'Voice name', type: 'text', value: 'zh-TW-HsiaoChenNeural' }, ...ttsFields.filter(f => !['model','voice'].includes(f.name))], responseType: 'blob', headers: { 'Content-Type': 'application/ssml+xml', 'X-Microsoft-OutputFormat': 'audio-24khz-48kbitrate-mono-mp3' } }
];
