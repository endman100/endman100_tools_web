...
const MODEL_REPO = "endman100/stable-diffusion-v1-5-onnx";
const HF_MODEL_BASE = `https://huggingface.co/${MODEL_REPO}/resolve/main/`;
const MANIFEST_URL = `${HF_MODEL_BASE}manifest.json`;
const CACHE_NAME = "endman100-sd15-onnx-web-v1";
const OPFS_DIR = "endman100-sd15-onnx-webgpu";
const DOWNLOAD_CHUNK_SIZE = 32 * 1024 * 1024;
const MAX_WEBGPU_ATTENTION_TOKENS = 4096;
const MIN_QUALITY_STEPS = 8;
const DEFAULT_QUALITY_STEPS = 40;
const MAX_QUALITY_STEPS = 60;
const RANDOM_SEED_SENTINEL = -1;
const MODEL_MODULES = ["text_encoder", "unet", "vae_encoder", "vae_decoder"];
  ...
  return [
    {
      moduleName,
      role: "model",
      path: moduleInfo.model,
      bytes: moduleInfo.bytes ?? 0,
      label: `${moduleName} model`
    },
    ...(moduleInfo.externalData ?? []).map((entry, index) => ({
      moduleName,
      role: "external",
      path: entry.path,
      bytes: entry.bytes ?? 0,
...
function allModelAssetDefs() {
  return MODEL_MODULES.flatMap(moduleName => {
...
async function fetchCached(def) {
  ...
  if (!buffer) throw new Error(`Model file was not cached: ${def.path}`);
  return buffer;
}
...
async function loadSidecars() {
  if (state.tokenizer && state.scheduler) return;
  const [vocab, merges, scheduler] = await Promise.all([
...
async function warmModelCache() {
  ...
  state.cacheWarming = (async () => {
    ...
    cacheState.textContent = "model cached";
    setStatusBadge("success");
    setDot(dotModel, "done");
    setReady("Ready");
    hideProgress();
    log(`Model files cached from ${MODEL_REPO}. Sessions will initialize when Generate is clicked.`);
...
function sessionOptions(moduleInfo, externalBuffers = new Map()) {
  const externalData = (moduleInfo.externalData ?? []).map(entry => ({
    path: entry.path,
...
async function loadAll() {
  ...
  state.loading = (async () => {
    ...
    await loadManifest();
    await loadSidecars();
    cacheState.textContent = "model ready";

    const sessions = {};
    sessions.text = await createSession("text_encoder");
    sessions.unet = await createSession("unet");
    sessions.vaeEncoder = await createSession("vae_encoder");
    sessions.vae = await createSession("vae_decoder");
    ...
    setReady("Ready");
    hideProgress();
    log(`All ONNX sessions loaded from ${MODEL_REPO}.`);
    generateButton.disabled = false;
    return sessions;
  })();
...
function normalizeDimension(value) {
  ...
  return Math.max(128, Math.min(1024, Math.round(parsed / 64) * 64));
}
...
function validateSettings(settings) {
  if (settings.attentionTokens > MAX_WEBGPU_ATTENTION_TOKENS) {
    ...
    throw new Error(
      `This raw SD1.5 ONNX UNet would allocate self-attention for ${settings.attentionTokens} latent tokens (${latentShape}). ` +
      "Use 512x512 or smaller, or export an optimized attention UNet for larger sizes."
    );
  }
...
async function encodeSourceLatents(settings) {
  ...
  for (let i = 0; i < latentSize; i++) out[i] = tensor.data[i] * 0.18215;
  return out;
}
...
async function clearModelCache() {
  ...
  if (state.manifest) {
    for (const def of allModelAssetDefs()) await removeOpfsFile(def);
  }
...
function wireEvents() {
  ...
  loadButton.addEventListener("click", () => loadAll().catch(error => {
    setStatusBadge("error");
